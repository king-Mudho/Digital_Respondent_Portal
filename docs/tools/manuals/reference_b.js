...
const screens = [
  {
    ...
    rows: [
      ...
      ["Accept", "Records that the submission passed quality checking.", "Only a person can pass a questionnaire; even one that triggered no rule is not passed until someone accepts it. If the case has a locked pre-interview profile, every fact on it must first be verified with the respondent and reconciled (or a deviation recorded).", "The submission is marked QA passed, the case moves on, and the decision is audited. If the profile is not reconciled you are told how many facts are outstanding, and nothing changes.", w],
  ...
  },
  {
    ...
    rows: [
      ...
      ["Coding status (NOT_STARTED, IN_PROGRESS, COMPLETE)", "Record how far the coding has got. It moves to COMPLETE by itself when the completed KII Guide is synced for this KII ID.", "Shows what has and has not been analysed. If the interview had a locked pre-interview profile, COMPLETE is refused until it is reconciled; a synced KII Guide then completes it at the next sync.", "The status changes and cannot go backwards. If the profile is not reconciled you are told what is outstanding.", w],
