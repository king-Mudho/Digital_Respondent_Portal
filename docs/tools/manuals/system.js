...
module.exports = {
  ...
  blocks: [
    ...
    ["bullets", [
      ...
      "The KII Guide and the Document Analysis Tool are synchronised the same way: the portal keeps its own copy of every submission of all three forms and shows on **Form PDFs** whether it matches KoboToolbox. A completed KII Guide marks that interview's coding COMPLETE; a document coding deleted in KoboToolbox unlocks the document.",
      "Full answers are stored privately on the server, never in a web-accessible folder, and are included in backups.",
      "Completed forms can be downloaded as readable PDFs, emailed, and (PI only) exported in full for analysis. The portal keeps its own synchronised copy of all three KoboToolbox forms, and shows whether it matches KoboToolbox.",
    ]],
    ...
    ["bullets", [
      ...
      "**Document records** need an authenticity assessment (Verified or Disputed) before they can be Included; an interpretive memo records reasoning. A document is best created by uploading it (**Quick add**): the AI reads the file, fills in the record's details and drafts the coding form, and a person reviews it. The source file is attached to the record (and can be removed if the wrong one was uploaded). Coding happens in the KoboToolbox Document Analysis Tool: **Code this document** opens it with the DOC ID filled in, and **Auto-fill** has AI draft the whole form from the source file for a Documentary RA to review, edit and submit. A PDF over 100 pages is coded by page range (for example one chapter) or read whole in parts and combined into one draft. The AI never submits on its own, and every draft is audited.",
    ]],
    ...
    ["h2", "6.10 Form PDFs"],
    ["p", "Completed KoboToolbox forms for your role's form, newest first. The PI, Field Coordinator and Supervisor choose the form from **Form**. A panel at the top shows whether the portal and KoboToolbox hold the same number of submissions, and **Sync now** brings the portal's copy level. The PI also has **Excel workbook** and **All as PDFs (ZIP)** buttons here."],
