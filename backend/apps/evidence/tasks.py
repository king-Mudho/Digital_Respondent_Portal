...
def _fail(document: DocumentRecord, message: str) -> None:
    ...
    document.save(update_fields=["ai_draft_status", "ai_draft_error"])


@shared_task
def generate_ai_draft(document_pk: int, user_id: int | None = None, page_range: str = "") -> None:
    ...
    try:
        ...
        draft = generate_draft(
            ...
            user=user, page_range=page_range,
        )
    ...
    log_action("document.ai_draft_generated", document, {
        "model": document.ai_draft_model, "user_id": user_id, "pages": page_range or "all",
    })
